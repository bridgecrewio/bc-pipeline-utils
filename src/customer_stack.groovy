...
class CustomerStack {
    ...
    def signup(String aws_profile_customer, String email, String firstName, String lastName, String phone) {
            ...
            def createStackRes = this.script.sh(
                    ...
                    aws cloudformation create-stack --region ${this.region} --profile ${
                        aws_profile_customer
                                    ...
                                    --parameters ParameterKey=ResourceNamePrefix,ParameterValue=${paramResourceNamePrefix} ParameterKey=ExternalID,ParameterValue=${paramExternalID} ParameterKey=CustomerName,ParameterValue=${paramCustomerName} ParameterKey=CreateTrail,ParameterValue=No ParameterKey=ExistingTrailBucketName,ParameterValue=${outputFormat.BucketArn} ParameterKey=ExistingTrailTopicArn,ParameterValue=${outputFormat.TopicARN}
                """,
                    returnStdout: true
            ).trim();
